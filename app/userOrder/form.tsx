...
const BrandOrderForm: React.FC<Props> = ({ datasource, createSource }) => {
  ...
  const skuFilter = (skus: Sku[]) => {
    const _skus = skus.filter((sku) => {
      const _s = brands.some(
        (x: any) =>
          selectedBrand?._id === x?._id &&
          x.spus.some((k) => k._id === sku?.spu?.spuId)
      );

      // const s = brands.find((_b: any) => {
      //   console.log(_b.spus, "spus")
      //   return _b.spus.some((_s: any) => _s._id === sku.spu.spuId);
      // });
      return _s;
    });
    // debugger;
  ...
  return (
    <article className="flex flex-row">
      ...
      >
        ...
        {brandOrders
          // .filter((o) => o._id === selectedBrand?._id)
          .map((order, index) => (
            <motion.div
              key={index}
